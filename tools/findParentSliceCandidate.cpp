...
bool hasRegisterInPost(frame &cur_frame, uint64_t target) {
	...
	for (int i = 0; i < operand_list.elem_size(); i++) {
		...
		//if (cur_specific.reg_operand().name().compare(reg) == 0) return true;

	}
	return false;
}

void print_all(const char *f, uint64_t frameNo, uint64_t target) {
	TraceContainerReader t(f);

	ctr = frameNo;
	std::auto_ptr<frame> cur_frame;
	...
	while (ctr > 0) {
		...
		if (hasRegisterInPost(*cur_frame, target)) {
			return;
		}
...
int main(int argc, char **argv)
{
	if (argc != 4) {
		if (argv[0]) {
			std::cout << "Usage: " << argv[0] << " <trace> <frame no of fork> <memorylocationofvariable>" << std::endl;
		}
		exit(1);
	}
	...
	print_all(argv[1],frameNo,target);
}
