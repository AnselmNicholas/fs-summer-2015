...
void print_element(const ::operand_info& cur_element)
{
	...
	const ::operand_usage& cur_usage = cur_element.operand_usage();
	std::cout << "\t[";
	if(cur_usage.read())	std::cout << " r";
	if(cur_usage.written())	std::cout << " w";
	if(cur_usage.index())	std::cout << " i";
	if(cur_usage.base())	std::cout << " b";
	std::cout << " ]";
...
//std::list<uint64_t> get_written_memory(frame &cur_frame)
//{
//	const ::std_frame& cur_std_frame = cur_frame.std_frame();
//
...
//		{
//
//			cur_std_frame.operand_post_list().elem(i)
//			const ::operand_info_specific& cur_specific = cur_element.operand_info_specific();
//			if(cur_specific.has_reg_operand())
//				std::cout << cur_specific.reg_operand().name();
//			else if(cur_specific.has_mem_operand())
...
//uint64_t getNWrittenMemory(frame &cur_frame,uint64_t ele) {
//
//	const ::std_frame& cur_std_frame = cur_frame.std_frame();
//
//	if (cur_std_frame.has_operand_post_list()) {
//
//		const ::operand_info_specific& cur_specific = cur_std_frame.operand_post_list().elem(ele).operand_info_specific();
//
//		return cur_specific.mem_operand().address();
//
//	}
//	return 0;
//}
...
void print_all(const char *f, uint64_t src,uint64_t ele, uint64_t dst)
{
...
//	print_memory_values(*cur_frame);
	...
	//uint64_t memLoc = getNWrittenMemory(*cur_frame,ele);
	...
	while (ctr <= dst) {
		...
		if (cur_frame->has_std_frame()) {
			...
			if (isAddressOverwritten(*cur_frame,ele)) {
				std::cout << "False " << ctr - 1 << std::endl;
//				print_memory_values(*cur_frame);
				return;
...
int main(int argc, char **argv)
{
	if (argc != 5) {
		if (argv[0]) {
			std::cout << "Usage: " << argv[0] << " <trace> <sourceInsn> <ele1> <dstInsn> " << std::endl;
		}
		exit(1);
	}

	char *end;
	uint64_t src = strtoul(argv[2], &end, 10);
	uint64_t ele = strtoull(argv[3], &end, 16);
	uint64_t dst = strtoul(argv[4], &end, 10);

	xed_tables_init();
	//std::cout << frameNo << " " << paramCnt << std::endl;
	print_all(argv[1],src,ele,dst);
}
